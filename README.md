...
개인 VM 환경에서 Ambari 같은 통합 관리 도구 없이, Ansible playbook과 role 구조를 통해 서버 공통 설정, Java 설치, Bigtop repository 등록, Hadoop/HDFS/YARN/Spark 설치, 설정 파일 배포, 서비스 기동, 모니터링 및 알림 구성을 자동화하는 것을 목표로 합니다.
...
현재는 HDFS, YARN, Spark on YARN, Prometheus/Grafana 기반 모니터링, JMX Exporter 기반 JVM 메트릭 수집, Alertmanager 기반 알림 구성을 완료한 상태입니다.
...
Prometheus `/alerts`와 Alertmanager UI에서 alert 상태를 확인합니다.
